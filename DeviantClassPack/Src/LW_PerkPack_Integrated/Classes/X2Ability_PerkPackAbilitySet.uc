...
simulated function DoubleTap1_BuildVisualization(XComGameState VisualizeGameState)
{
  ...
  // TODO: Fix this part.  Looks like it's trying to strip everything up to the back-to-cover move so it can populate another shot.
...
function CombatRush_BuildVisualization(XComGameState VisualizeGameState)
{
  ...
  local XComGameStateContext_Ability		context;
  local StateObjectReference				InteractingUnitRef;
  local VisualizationActionMetadata				EmptyTrack, ActionMetadata, TargetMetadata;
  ...
  foreach VisualizeGameState.IterateByClassType(class'XComGameState_Effect', EffectState)
  {
    if (EffectState.ApplyEffectParameters.EffectRef.SourceTemplateName == 'BroadcastCombatRush')
    {
      ...
      if (UnitState != none && EffectState.StatChanges.Length > 0 && InteractingUnitRef.ObjectID != UnitState.ObjectID)
      {
        ...
        TargetMetadata.StateObject_OldState = History.GetGameStateForObjectID(UnitState.ObjectID, eReturnType_Reference, VisualizeGameState.HistoryIndex - 1);
        TargetMetadata.VisualizeActor = UnitState.GetVisualizer();
        SoundandFlyoverTarget = X2Action_PlaySoundAndFlyOver(class'X2Action_PlaySoundAndFlyOver'.static.AddToVisualizationTree(TargetMetadata, VisualizeGameState.GetContext(), false, TargetMetadata.LastActionAdded));
